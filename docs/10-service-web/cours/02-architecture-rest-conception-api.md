...
!!! tip "Présentation de ce chapitre"
    [🖥 Ouvrir les diapositives](../../presentation.html?deck=10-service-web/slides/02-architecture-rest-conception-api.txt){ target=_blank }
...
|---|---|
| `GET /commandes/17` | `GET /getCommande/17` |
| `POST /commandes` | `POST /commandes/creer` |
| `DELETE /commandes/17` | `POST /commandes/17/supprimer` |
| `GET /utilisateurs/42/commandes` | `GET /commandes?utilisateur=42&action=liste` |
...
|---|---|---|---|
| Dans l'URI | `/api/v2/commandes` | Simple, visible, facile à router | « Pollue » l'URI d'une information qui n'est pas, au sens strict, une propriété de la ressource |
| En-tête personnalisé | `X-API-Version: 2` | URI stable | Moins visible, nécessite une documentation explicite |
| Négociation de contenu | `Accept: application/vnd.exemple.v2+json` | Conforme à l'esprit HTTP (négociation de représentation) | Plus complexe à mettre en œuvre et à documenter côté client |
...
@app.route("/commandes/<int:id_commande>", methods=["PATCH"])
def modifier_commande(id_commande):
    if id_commande not in commandes:
        return jsonify({"erreur": "Commande introuvable"}), 404
    commandes[id_commande].update(request.get_json())
    return jsonify(commandes[id_commande]), 200
```
