...
!!! tip "Présentation de ce chapitre"
    [🖥 Ouvrir les diapositives](../../presentation.html?deck=10-service-web/slides/04-authentification-autorisation-api.txt){ target=_blank }
...
| Acteur | Rôle |
|---|---|
| **Propriétaire de la ressource** (*resource owner*) | L'utilisateur final, propriétaire des données |
...
Un **JWT** (*JSON Web Token*) est un format standardisé de jeton compact, auto-porteur (*self-contained*), fréquemment utilisé comme jeton d'accès dans les architectures OAuth 2.0 ou comme mécanisme d'authentification autonome. Un JWT se compose de trois parties encodées en Base64URL, séparées par des points :
...
- Toujours définir une expiration (`exp`) courte, et prévoir un mécanisme de jeton de rafraîchissement (*refresh token*) pour renouveler l'accès sans réauthentification complète.
- Ne jamais faire confiance à l'algorithme annoncé dans l'en-tête du jeton reçu (imposer explicitement, côté serveur, l'algorithme attendu lors de la vérification) — une vulnérabilité historiquement rencontrée dans plusieurs bibliothèques JWT consistait précisément à faire confiance à un en-tête `alg` modifié par un attaquant (ex. bascule vers `none`).
- Prévoir un mécanisme de révocation (liste de révocation, ou jetons de courte durée de vie combinés à un rafraîchissement contrôlé), car un JWT signé reste valide jusqu'à son expiration même si le compte associé est désactivé entre-temps.
- Stocker la clé de signature (ou la clé privée en cas de signature asymétrique) hors du code source, dans un gestionnaire de secrets dédié.
