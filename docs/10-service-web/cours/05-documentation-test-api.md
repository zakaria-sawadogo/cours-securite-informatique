...
!!! tip "Présentation de ce chapitre"
    [🖥 Ouvrir les diapositives](../../presentation.html?deck=10-service-web/slides/05-documentation-test-api.txt){ target=_blank }
...
## 2. La spécification OpenAPI

**OpenAPI** (anciennement connue sous le nom de Swagger, avant que sa spécification ne soit cédée à l'*OpenAPI Initiative*) est aujourd'hui la spécification de référence pour décrire formellement une API REST au format JSON ou YAML : ses ressources, ses opérations, ses paramètres, les schémas de ses corps de requête et de réponse (en s'appuyant sur JSON Schema, chapitre 3), ses codes de statut possibles, et ses mécanismes d'authentification.
...
components:
  schemas:
    Commande:
      type: object
      required: [id, statut, montant]
      properties:
        id:
          type: integer
        statut:
          type: string
          enum: [en_preparation, expediee, livree, annulee]
        montant:
          type: number
```
...
@app.get("/commandes/{id_commande}", response_model=Commande)
def obtenir_commande(id_commande: int):
    return {"id": id_commande, "statut": "en_preparation", "montant": 45000}

# La spécification OpenAPI est disponible automatiquement sur /openapi.json,
# et une interface interactive Swagger UI sur /docs.
```

## 3. Stratégies de test d'une API

Comme pour tout logiciel, les tests d'une API se déclinent en plusieurs niveaux complémentaires, chacun ayant un objectif et un coût différents.

| Niveau | Objectif | Exemple |
|---|---|---|
| **Test unitaire** | Vérifier isolément une fonction ou un composant métier, sans passer par la couche HTTP | Vérifier qu'une fonction de calcul de remise applique correctement le taux attendu |
| **Test d'intégration** | Vérifier le comportement de l'API à travers de véritables requêtes HTTP, généralement avec une base de données de test | Envoyer une requête `POST /commandes` et vérifier la réponse et l'état de la base |
| **Test de contrat** (*contract testing*) | Vérifier que l'implémentation respecte fidèlement le contrat documenté (OpenAPI) | Valider automatiquement chaque réponse réelle contre le schéma déclaré |
| **Test de bout en bout** (*end-to-end*) | Vérifier un scénario complet impliquant plusieurs services ou composants | Créer un compte, s'authentifier, passer une commande, vérifier la facturation |
...
Un test de contrat vérifie que chaque réponse réelle de l'API respecte le schéma JSON Schema déclaré dans la spécification OpenAPI, ce qui permet de détecter automatiquement une dérive entre le comportement réel de l'implémentation et sa documentation officielle — un problème fréquent lorsque documentation et code évoluent séparément.
...
# La validation compare la réponse HTTP réelle au schéma déclaré pour cette route
# et lève une erreur en cas d'écart (champ manquant, type incorrect, code de statut
# non documenté).
```
