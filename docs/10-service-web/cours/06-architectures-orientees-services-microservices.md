...
!!! tip "Présentation de ce chapitre"
    [🖥 Ouvrir les diapositives](../../presentation.html?deck=10-service-web/slides/06-architectures-orientees-services-microservices.txt){ target=_blank }
...
def sur_commande_creee(evenement):
    envoyer_email_confirmation(evenement["id"])
```

| Aspect | Synchrone (REST, gRPC) | Asynchrone (file de messages) |
|---|---|---|
| Couplage temporel | Fort (l'appelé doit être disponible immédiatement) | Faible (traitement différé possible) |
| Simplicité de raisonnement | Plus simple à suivre (un appel, une réponse) | Plus complexe (traçabilité, ordre de traitement) |
| Cas d'usage typique | Requête nécessitant une réponse immédiate pour l'utilisateur | Traitement différé, notification, propagation d'un événement à plusieurs consommateurs |
...
- **Routage** des requêtes entrantes vers le microservice approprié.
...
```
                        ┌──────────────────┐
Client  ───requête───►  │   API Gateway    │
                        └──────────────────┘
                          │        │        │
                          ▼        ▼        ▼
                    Service    Service   Service
                  utilisateurs commandes  paiement
```
